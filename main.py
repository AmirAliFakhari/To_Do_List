    ...
    while True:
        ...
        try:
            if choice == "1":
                ...
                project_id_str: str = input(
                    "Enter project ID to add task to: "
                ).strip()
                project_id: int = int(project_id_str)
                ...
                print(
                    f"✅ SUCCESS: Task '{task.title}' added to project ID {project_id}."
                )
                ...
                new_status: str = input(
                    "New status (todo/doing/done) [blank to keep]: "
                ).strip()
                new_deadline_str: str = input(
                    "New deadline (YYYY-MM-DD) [blank to keep]: "
                ).strip()
                ...
                project_id_str: str = input(
                    "Enter project ID to list tasks for: "
                ).strip()
                project_id: int = int(project_id_str)
